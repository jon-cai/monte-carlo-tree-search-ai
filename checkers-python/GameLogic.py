...
class GameLogic:
    ...
    def Run(self,fh=None,**kwargs):
        if self.mode == 'n' or self.mode == 'network' :
            if kwargs['mode'] == 'host':
                self.ai_list.append(
                    IOAI(self.col, self.row, self.p, ai_path=kwargs['ai_path'], time=kwargs['halftime']))
                self.ai_list.append(
            ...
            self.ai_list.append(
                IOAI(self.col, self.row, self.p, ai_path=kwargs['ai_path_1'], time=kwargs['halftime']))
            self.ai_list.append(
                IOAI(self.col, self.row, self.p, ai_path=kwargs['ai_path_2'], time=kwargs['halftime']))
